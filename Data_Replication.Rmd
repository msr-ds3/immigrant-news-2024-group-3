...
ggplot() +
  ...
  geom_smooth(data = migrant_plot_data, aes(x = date, y = Number, color = president), se = FALSE) +
  scale_y_continuous(name = "Number", limits = c(0, 150000)) +
  geom_smooth(data = documents_plot_data, aes(x = month, y = border_prop * 1500, group = time), se = FALSE, inherit.aes = FALSE) +
  geom_vline(data = line_plot_data, aes(xintercept = line_date), linetype = "dashed") +
  scale_y_continuous(
    name = "Number",
    limits = c(0, 150000),
    sec.axis = sec_axis(~./1500, name = "Border Prop")
  ) +
  theme_minimal() + 
  theme(
...
```
Although we expected the number of immigrants apprehended at the border to have a larger effect than it did on the porportion of times the media discussed immigration at the border, this was not present in our regression. 
